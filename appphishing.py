    ...
    if db_pool is None:
        try:
            db_pool = ConnectionPool(
                conninfo=(
                    f"host=db.ezsqvlwxqiwewidquvwp.supabase.co "
                    f"dbname=postgres user=postgres "
                    f"password={os.environ.get('SUPABASE_PASSWORD')} port=5432"
                ),
                min_size=1,
                max_size=10,
                timeout=30
            )
            logging.info("Supabase PostgreSQL Connection Pool initialized with psycopg3.")
...
def return_db_connection(conn):
    ...
    try:
        ...
        logging.error(f"Error returning connection: {err}")
